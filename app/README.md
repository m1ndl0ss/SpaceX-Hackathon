...
The default simulation runs entirely in the frontend. Eight hardcoded project profiles in `src/js/demo-impact.js` produce repeatable results that respond to project size, resource inputs, assessment year, nearby projects, and mitigation options. Saved results keep the preset version and exact outputs. The collapsed simulation details identify this as demo mode; no model evaluation statistics or invented site records are attached to these results.

`npm run build` works from `app` alone using the checked-in catalog. It does not need Python, `ai/`, `data/`, a model API, or new deployment settings. Existing frontend deployments can use the same build command and `dist` output. `VITE_IMPACT_MODE` defaults to `demo` when unset.

## Optional trained-model mode

To use the team's Python models locally instead of frontend presets:

```bash
npm run models:setup
npm run models:sync
VITE_IMPACT_MODE=api npm run dev
```
...
`npm run models:sync` manually refreshes selectors, artifact fingerprints, and evaluation metadata from the supplied model files. An external service must use the matching model artifacts. If building with `VITE_IMPACT_MODE=api`, run the model service separately and route `/api/impact/*` to it with the prefix removed; API mode does not silently fall back to demo results. The default frontend demo needs no API routing.
